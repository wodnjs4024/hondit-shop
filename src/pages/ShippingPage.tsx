    ...
    <V23Page>
      <main className="v23-shipping-page">
        <V23PageHero
          ...
          title={
            market.hasShopee
              ? marketText(language, "Two order routes. Two clear delivery windows.", "두 가지 주문 방식, 명확한 배송 흐름.")
              : marketText(language, "Bulk checkout. One clear delivery route.", "대량주문 전용, 하나의 배송 경로.")
          }
          description={
            market.hasShopee
              ? marketText(
              ...
              : marketText(
                  language,
                  `${countryName} does not use a Shopee retail route. Confirmed bulk shipments are prepared after PayPal capture and dispatched from Korea.`,
                  `${countryName}에는 Shopee 개별 구매 경로를 사용하지 않습니다. 대량주문은 PayPal 결제 완료 후 한국에서 준비 및 발송됩니다.`,
                )
          }
        ...
        <section className={`v23-shipping-compare ${market.hasShopee ? "" : "is-single-route"}`}>
          {market.hasShopee && (
            <article>
              ...
              <h2>{marketText(language, "Shop through Shopee Singapore", "Shopee 싱가포르에서 구매")}</h2>
              <ul>
          ...
          <article>
            ...
            <ul>
              ...
              <li>{marketText(language, "Server-verified PayPal capture before an admin order is confirmed", "서버에서 PayPal 결제 완료가 검증된 주문만 관리자 주문으로 확정")}</li>
            </ul>
        ...
        <section className="v23-timing-notes">
          ...
          <article><b>{marketText(language, "Customs and carrier handover", "통관 및 배송사 인계")}</b><p>{marketText(language, "Inspection, flight capacity and local handover can add time.", "검사, 항공편, 현지 인계 상황에 따라 시간이 추가될 수 있습니다.")}</p></article>
        ...
        <section className="v23-route-banner">
          ...
          <p>
            {market.hasShopee
              ? marketText(language, "Use Shopee chat for a Shopee transaction, or prepare a message for the hondit team.", "Shopee 주문은 Shopee 채팅을, 대량주문은 hondit 문의를 이용해주세요.")
