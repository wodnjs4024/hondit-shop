...
import { marketCountryName, marketText, formatMarketMoney, useMarket } from "../lib/market";
    ...
    <V23Page>
      <main className="v23-bulk-page">
        <V23PageHero
          ...
          title={marketText(language, "Build the order. Pay securely with PayPal.", "주문을 구성하고 PayPal로 안전하게 결제하세요.")}
          description={marketText(
            language,
            `Choose a product, review its MOQ and ${market.currency} total, add ${countryName} delivery details and continue to PayPal. Secret keys stay on the server.`,
            `상품과 MOQ, ${market.currency} 결제 금액을 확인한 뒤 ${countryName} 배송 정보를 입력하고 PayPal로 결제합니다. 비밀키는 서버에만 보관됩니다.`,
          )}
        ...
        <section className="v23-bulk-steps">
          <article><span>01</span><b>{marketText(language, "Choose one product", "상품 선택")}</b><p>{marketText(language, "MOQ and approved quantity increments are checked automatically.", "MOQ와 주문 단위는 자동으로 확인됩니다.")}</p></article>
          ...
          <article><span>04</span><b>{marketText(language, "Manage the order", "관리자 확인")}</b><p>{marketText(language, "The result appears in hondit's protected admin console.", "결제 완료 주문은 관리자 페이지에 표시됩니다.")}</p></article>
        </section>
            ...
            {products.map((product) => (
              <button key={product.slug} type="button" className={selected?.slug === product.slug ? "is-selected" : ""} onClick={() => setSelectedSlug(product.slug)}>
                ...
                <span>
                  ...
                  <em>{marketText(language, `${formatMarketMoney(product.bulkUnitPrice, market)} each - MOQ ${product.bulkMoq}`, `개당 ${formatMarketMoney(product.bulkUnitPrice, market)} - MOQ ${product.bulkMoq}`)}</em>
                </span>
              </button>
            ))}
          </div>
            ...
            {selected && (
              <>
                ...
                <dl>
                  ...
                  <div><dt>{marketText(language, "Unit price", "개당 가격")}</dt><dd>{formatMarketMoney(selected.bulkUnitPrice, market)}</dd></div>
                  <div><dt>{marketText(language, "Minimum total", "최소 결제액")}</dt><dd>{formatMarketMoney(selected.bulkUnitPrice * selected.bulkMoq, market)}</dd></div>
                  <div><dt>{marketText(language, "Delivery", "배송")}</dt><dd>{marketText(language, market.checkoutNote, market.checkoutNoteKo)}</dd></div>
                </dl>
                <Link to={`/bulk-orders/${selected.apiSlug}`}>{marketText(language, "Continue to delivery and PayPal ->", "배송 입력 및 PayPal 결제로 이동 ->")}</Link>
              </>
            )}
          </aside>
        </section>
        ...
        <section className="v23-route-banner">
          <h2>{marketText(language, "Need a mixed commercial order?", "여러 상품을 섞어 주문해야 하나요?")}</h2>
