...
export function ContactPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage(data.referenceNumber
      ...
      trackEvent("submit_inquiry", { inquiry_channel: "contact_page", market: market.code, currency: market.currency });
      ...
      <main className="v23-contact-page">
        <section className="v23-contact-hero">
          <div>
            ...
            <h1>{marketText(language, "A real inbox,\nwith a clear route for every question.", "문의가 운영팀에\n정확히 도착하도록.")}</h1>
            <p>{marketText(language, `Send a message directly to the hondit admin inbox for ${countryName} orders, or choose the fastest external channel for an existing Shopee order.`, `${countryName} 주문 문의는 hondit 관리자 문의함으로 바로 저장됩니다. 이미 진행 중인 Shopee 주문은 가장 빠른 외부 채널을 선택해주세요.`)}</p>
          </div>
        ...
        <section className="v23-contact-grid">
          <form onSubmit={submit}>
            ...
            <p>{marketText(language, `This form is saved to hondit's protected admin inbox with the ${countryName} market tag.`, `이 문의는 ${countryName} 판매판 정보와 함께 hondit 관리자 문의함에 저장됩니다.`)}</p>
            ...
            <div className="v23-form-two">
              ...
              <label>{marketText(language, "Order number", "주문번호")}<input name="orderNumber" placeholder="Shopee or HON reference" /></label>
            </div>
            ...
            <label>{marketText(language, "Message", "문의 내용")} *<textarea name="message" required placeholder={marketText(language, "Include the product, order reference or question so we can help quickly.", "상품명, 주문번호, 질문 내용을 함께 적어주시면 더 빠르게 도와드릴 수 있습니다.")} /></label>
          ...
          <aside>
            ...
            <a href={SHOPEE} target="_blank" rel="noreferrer"><small>EXISTING SHOPEE ORDER</small><b>Shopee Chat</b><span>{marketText(language, "Payment, voucher, delivery tracking or address changes for Shopee orders. ↗", "Shopee 주문 결제, 쿠폰, 배송 추적, 주소 변경 문의. ↗")}</span></a>
            <a href={INSTAGRAM} target="_blank" rel="noreferrer"><small>PRODUCT AND SOCIAL</small><b>Instagram</b><span>{marketText(language, "Short product questions, social content and informal collaborations. ↗", "짧은 상품 문의, 소셜 콘텐츠, 협업 문의. ↗")}</span></a>
            <a href={`mailto:${EMAIL}`}><small>FORMAL DOCUMENTS</small><b>Email</b><span>{marketText(language, "Attachments and formal records can still be sent by email.", "첨부파일이나 공식 문서는 이메일로 보낼 수 있습니다.")}</span></a>
            <a href="/bulk-orders"><small>DIRECT BULK ORDER</small><b>PayPal checkout</b><span>{marketText(language, "Choose an MOQ quantity and create a tracked order. →", "최소 주문 수량을 선택하고 추적 가능한 주문을 생성합니다. →")}</span></a>
          </aside>
        </section>
      </main>
    </V23Page>
